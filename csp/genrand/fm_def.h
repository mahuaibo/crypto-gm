...
#define FME_ECC_KEYPAIRNOMATCH  0x189      /* ECC pubkey and prikey not match */
...
#define FME_USER_LOCK           0x290      /* user is lock */
...
#define FM_ECC_MAX_INBUF_LEN2 4096
...
typedef struct tag_ECC_Cipher_st
{
    ...
    FM_U8  C[FM_ECC_MAX_INBUF_LEN2];             /*加密数据*/
    FM_U8  M[FM_ECC_MAX_LEN];  /*预留,用于支持带MAC输出的ECC算法*/
}FM_ECC_Cipher, *PFM_ECC_Cipher;//支持最大明文长度4096

typedef struct tag_ECC_Cipher_st0
{
    FM_U32 Clength;            /*加密数据长度*/
    FM_U8  x[FM_ECC_MAX_LEN];  /*与y组成椭圆曲线上的点(x,y)*/
    FM_U8  y[FM_ECC_MAX_LEN];  /*与x组成椭圆曲线上的点(x,y)*/
    ...
    FM_U8  M[FM_ECC_MAX_LEN];  /*预留,用于支持带MAC输出的ECC算法*/
}FM_ECC_Cipher0, *PFM_ECC_Cipher0; 
